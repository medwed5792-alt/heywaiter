    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: `QR для стола ${table.number}`,
          url,
        });
      } catch (e) {
...
export default function HallQRPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (venueSnap.exists() && venueSnap.data().venueType) {
        setVenueType(venueSnap.data().venueType as VenueType);
      }
      if (venueSnap.exists() && venueSnap.data().messages) {
        ...
        setMessages((prev) => ({
          checkIn: m.checkIn ?? prev.checkIn,
          booking: m.booking ?? prev.booking,
          thankYou: m.thankYou ?? prev.thankYou,
        }));
      }
  ...
  const loadHallsAndTables = async () => {
    const [hallsSnap, tablesSnap] = await Promise.all([
      getDocs(hallsRef),
      getDocs(tablesRef),
    ]);
    const hallList = hallsSnap.docs.map((d) => ({ id: d.id, name: (d.data().name as string) ?? "", order: (d.data().order as number) ?? 0 }));
  ...
  const deleteHall = async (hall: Hall) => {
    ...
    try {
      const inHall = tables.filter((t) => t.hallId === hall.id);
      for (const t of inHall) {
  ...
  return (
    <div className="max-w-4xl">
      ...
      <p className="mt-2 text-sm text-gray-600">
        Конструктор залов и столов, приветствий и QR-кодов. Данные сохраняются в Firestore под venueId.
      </p>

      <section className="mt-6 rounded-xl border border-gray-200 bg-white p-6">
        <h3 className="font-medium text-gray-900">Тип заведения</h3>
        <p className="mt-1 text-xs text-gray-500">
          Полный сервис — столы и вызов официанта; фастфуд — заказ по номеру и уведомление «Готово».
        </p>
        ...
        <button type="button" className="mt-3 rounded-lg bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-gray-800 disabled:opacity-50" onClick={handleSaveVenueType} disabled={venueTypeSaving}>
      ...
      <section className="mt-6 rounded-xl border border-gray-200 bg-white p-6">
        ...
        <button type="button" className="mt-4 rounded-lg bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-gray-800 disabled:opacity-50" onClick={handleSaveMessages} disabled={saving}>
      ...
      <section className="mt-6 rounded-xl border border-gray-200 bg-white p-6">
        ...
        <p className="mt-1 text-sm text-gray-500">Создайте залы (Веранда, Терраса, Основной зал, VIP) и добавляйте в них столы. QR ведёт на /check-in?v=venueId&t=tableId.</p>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          ...
          />
          <button type="button" className="inline-flex items-center gap-1 rounded-lg bg-gray-900 px-3 py-1.5 text-sm font-medium text-white hover:bg-gray-800" onClick={addHall}>
                ...
                <div key={hall.id} className="rounded-lg border border-gray-200 bg-gray-50/50 p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      {editingHall?.id === hall.id ? (
                        <input
                          ...
                          onKeyDown={(e) => {
                            if (e.key === "Enter") updateHall(hall);
                            if (e.key === "Escape") { setEditingHall(null); setEditingHallName(""); }
                          }}
                    ...
                    <div className="flex gap-1">
                      {editingHall?.id === hall.id ? (
                        <>
                          <button type="button" className="rounded border border-gray-300 px-2 py-1 text-xs text-gray-700 hover:bg-gray-100" onClick={() => updateHall(hall)}>Сохранить</button>
                          <button type="button" className="rounded border border-gray-300 px-2 py-1 text-xs text-gray-700 hover:bg-gray-100" onClick={() => { setEditingHall(null); setEditingHallName(""); }}>Отмена</button>
                        </>
                      ) : (
                        <>
                          <button type="button" className="rounded p-1 text-gray-500 hover:bg-gray-200" onClick={() => startEditHall(hall)} title="Редактировать зал"><Pencil className="h-3.5 w-3.5" /></button>
                          <button type="button" className="rounded p-1 text-gray-500 hover:text-red-600 hover:bg-red-50" onClick={() => deleteHall(hall)} title="Удалить зал"><Trash2 className="h-3.5 w-3.5" /></button>
                        </>
                      )}
                    </div>
                  </div>
                  {editingHall?.id !== hall.id && (
                    <div className="mt-3 flex flex-wrap items-center gap-2">
                      <button
                        ...
                        onClick={() => setAddingTableHallId(hall.id)}
                      >
                        <Plus className="h-3.5 w-3.5" /> Добавить стол
                      </button>
                    </div>
                  )}
                  {addingTableHallId === hall.id && (
                    <AddTableForm
                      hallId={hall.id}
                      onSave={(num, name, desc, seats) => addTable(hall.id, num, name, desc, seats)}
                      onCancel={() => setAddingTableHallId(null)}
                    />
                  )}
                  <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {hallTables.map((t) => (
                      <div key={t.id} className="flex items-center justify-between rounded border border-gray-200 bg-white p-3">
                        <div>
                          <p className="font-medium text-gray-900">Стол {t.number}{t.name ? ` · ${t.name}` : ""}</p>
                  ...
                  {editingTable && editingTable.hallId === hall.id && (
                    <EditTableForm
                      table={editingTable}
                      onSave={(num, name, desc, seats) => updateTable(editingTable, num, name, desc, seats)}
...
function AddTableForm({
  ...
  hallId: string;
  onSave: (number: number, name: string, description: string, seats: number) => void;
  onCancel: () => void;
}) {
  ...
  return (
    <div className="mt-3 rounded border border-gray-200 bg-white p-3 grid grid-cols-2 sm:grid-cols-4 gap-2">
      <label>
        <span className="block text-xs text-gray-600">Номер *</span>
        <input type="number" min={1} value={number} onChange={(e) => setNumber(e.target.value)} className="mt-1 w-full rounded border border-gray-300 px-2 py-1.5 text-sm" />
      </label>
      <label>
        <span className="block text-xs text-gray-600">Название</span>
        <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Опционально" className="mt-1 w-full rounded border border-gray-300 px-2 py-1.5 text-sm" />
      </label>
      <label>
        <span className="block text-xs text-gray-600">Мест</span>
        <input type="number" min={0} value={seats} onChange={(e) => setSeats(e.target.value)} className="mt-1 w-full rounded border border-gray-300 px-2 py-1.5 text-sm" />
      </label>
...
function EditTableForm({
  ...
  return (
    <div className="mt-3 rounded border border-gray-200 bg-white p-3 grid grid-cols-2 sm:grid-cols-4 gap-2">
      ...
      <label>
        <span className="block text-xs text-gray-600">Название</span>
        <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="mt-1 w-full rounded border border-gray-300 px-2 py-1.5 text-sm" />
      </label>
      <label>
        <span className="block text-xs text-gray-600">Мест</span>
        <input type="number" min={0} value={seats} onChange={(e) => setSeats(e.target.value)} className="mt-1 w-full rounded border border-gray-300 px-2 py-1.5 text-sm" />
