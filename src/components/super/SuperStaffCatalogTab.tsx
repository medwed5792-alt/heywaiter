...
export function SuperStaffCatalogTab() {
  ...
  const handleSaveKarma = async (userId: string) => {
    try {
      ...
      setUsers((prev) => prev.map((u) => (u.id === userId ? { ...u, globalScore: editScore } : u)));
      setEditingId(null);
  ...
  const handleDelete = async (userId: string) => {
    if (
      !confirm(
        "Полностью удалить этого сотрудника из системы? Это удалит запись в global_users и все связи с заведениями."
      )
    )
      return;
  ...
  const displayName = (u: GlobalUser) =>
    [u.firstName, u.lastName].filter(Boolean).join(" ") || u.identity?.displayName || u.id;
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-sm text-slate-600">
        Весь список людей в системе (global_users). Супер-админ может редактировать рейтинг (карму) и полностью удалять
        запись.
      </p>
                  ...
                  <tr key={u.id} className="border-b border-slate-100">
                    <td className="p-3">
                      <div className="flex items-center gap-2">
                        <div className="h-10 w-10 shrink-0 overflow-hidden rounded-full bg-slate-200 flex items-center justify-center">
                          {u.photoUrl ? (
                    ...
                    <td className="p-3 max-w-xs truncate text-sm text-slate-600" title={affiliationsText(u)}>
                      {affiliationsText(u)}
                    </td>
                    <td className="p-3">
                      {editingId === u.id ? (
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            onClick={() => void handleSaveKarma(u.id)}
                          >
                            OK
                          </button>
                          <button
